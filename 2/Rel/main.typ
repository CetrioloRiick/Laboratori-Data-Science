...
#figure(
  image("FunzioneCosto.svg", width: 120%),
...
#figure(
  image("FunzioneCosto2.svg", width: 120%),
...
#figure(
  image("batcsize tempo6.7e-3.svg", width: 120%),
...
#figure(
  image("batcsize epoche6.7e-3.svg", width: 120%),
...
In particolare, il metodo che converge più rapidamente è quello con `batch_size = 1`, cioè il caso in cui ogni aggiornamento viene effettuato utilizzando un singolo punto del dataset. Questo risultato può apparire inizialmente controintuitivo, poiché ci si potrebbe aspettare che un gradiente così rumoroso comprometta la convergenza complessiva; tuttavia evidentemente, nel caso in esame, la struttura del problema rende comunque possibile una convergenza.
...
QUESTA PARTE TIENIAMOLA PER DOPO

Dal punto di vista qualitativo, i batch più piccoli tendono inoltre a rendere il processo di discesa più rumoroso, poiché ogni aggiornamento viene calcolato su un sottoinsieme ridotto di dati. Al contrario, batch più grandi producono una traiettoria di convergenza più stabile e regolare, ma generalmente meno reattiva nelle fasi iniziali dell’ottimizzazione.